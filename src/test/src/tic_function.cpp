...
return;
    ...
    if (msg->data != pre_save_map.data)
    {
        if (msg->data)
        {
            system("sh /root/sh/save_map.sh");//运行保存地图命令
        }
...
void chatterCallback(const std_msgs::Float32MultiArrayPtr & msg)
{
    if(!ros::ok())
{
return;
}
   system("sh /home/ubuntu/sh/path_play.sh");
...
int main(int argc, char *argv[])
{
	...
	ros::Subscriber sub = n.subscribe("MappingOrNavi", 1000, whatCallback);
        ...
        ros::Subscriber sub_chatter = n.subscribe("/rect_aera", 10, chatterCallback);
	ros::spin();
